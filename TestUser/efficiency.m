...
function [msgLog voutOK effOK iinOK dropOK] = efficiency(model,effM, voutM, voutTested, voutValue, avg_vin, avg_iload_1, avg_iload_2)
...
try
    ...
    if(voutZERO == -1)
        ...
        if(abs(voutZERO) - abs(voutM(avg_vin,avg_iload_1)) <= abs(cursor.Data{4}))
        ...
        if(abs(voutZERO) - abs(voutM(avg_vin,avg_iload_2)) <= abs(cursor.Data{5}))
    ...
    if(abs((effM(avg_vin,avg_iload_1)*100)) >= abs(cursor.Data{6}))
    ...
    if(abs((effM(avg_vin,avg_iload_2)*100)) >= abs(cursor.Data{7}))
